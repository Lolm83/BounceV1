...
namespace Tmpl8
{
	...
	{
		int p_height = 17 * player->m_scalefactor, p_width = 10 * player->m_scalefactor;

		int ybottom = 3 * player->m_scalefactor;
		int ytop = 4 * player->m_scalefactor;
		int xleft2 = 6 * player->m_scalefactor;
		int xright2 = 8 * player->m_scalefactor;

		float x2 = 0, y2 = 0;
		
		// Right Foot
		if (player->m_x + xleft2 + p_width >= circle->m_x - circle->m_radius &&
			player->m_x <= circle->m_x - xleft2)
			x2 = player->m_x + xleft2 + p_width, y2 = player->m_y + p_height + ytop;

		// Left Foot
		else if (player->m_x + xleft2 > circle->m_x &&
			player->m_x <= circle->m_x + circle->m_radius - xleft2)
			x2 = player->m_x + xleft2, y2 = player->m_y + p_height + ytop;
		else
			return false;

		float x1 = circle->m_x, y1 = circle->m_y, r1 = circle->m_radius;

		if (player->m_y + ytop >= y1)
		{
			y2 = player->m_y + ytop;
			if (player->m_x > circle->m_x)
				x2 = player->m_x + xleft2;
			else
				x2 = player->m_x + xleft2 + p_width;
		}
